...
namespace Chess.Systems
{
    public class ViewSystem : ISystem
    {
        ...
        {
            if (Process.Instance.GetProcess(Process.Instance.GetShowPlayerId()) == ConstUtil.Process_Pick_Hero)
            {
                Debug.Log("ViewSystem Update - pick hero");
                AddHeroPickView();
                Process.Instance.SetProcess(ConstUtil.Process_Pick_Hero_Ing, Process.Instance.GetShowPlayerId());
            }
            else if (Process.Instance.GetProcess(Process.Instance.GetShowPlayerId()) == ConstUtil.Process_Prepare_Start)
            {
                AddBartenderView();
                AddPlayerView();
                Process.Instance.SetProcess(ConstUtil.Process_Prepare_Bartender_Refresh_Pre, Process.Instance.GetShowPlayerId());
            }
            else if (Process.Instance.GetProcess(Process.Instance.GetShowPlayerId()) == ConstUtil.Process_Prepare_Switch)
            {
