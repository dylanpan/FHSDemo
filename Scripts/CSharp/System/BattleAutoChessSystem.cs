    ...
    public class BattleAutoChessSystem : ISystem
    {
        ...
        public void GetBattleResult(int round, ref Entity entity)
        {
            ...
            Console.WriteLine("BattleAutoChessSystem DoBattle-----------------03");
                ...
                else if ((CommonUtil.Battle_CheckListAllSpecificStatus(new int[]{ConstUtil.Status_PieceDead,ConstUtil.Status_PieceNoAtk}, aList) && !CommonUtil.Battle_CheckListHaveStatus(ConstUtil.None, aList) && !CommonUtil.Battle_CheckListHaveStatus(ConstUtil.Status_PieceAtk, aList))
                    && (CommonUtil.Battle_CheckListAllSpecificStatus(new int[]{ConstUtil.Status_PieceDead,ConstUtil.Status_PieceNoAtk}, bList) && !CommonUtil.Battle_CheckListHaveStatus(ConstUtil.None, bList) && !CommonUtil.Battle_CheckListHaveStatus(ConstUtil.Status_PieceAtk, bList)))
                {
                    // 结束：同空，平局
                    resultComponent.status = ConstUtil.ResultStatus_Draw;
                }
                else if ((!CommonUtil.Battle_CheckListAllSpecificStatus(new int[]{ConstUtil.Status_PieceDead,ConstUtil.Status_PieceNoAtk}, aList) && !CommonUtil.Battle_CheckListHaveStatus(ConstUtil.None, aList) && !CommonUtil.Battle_CheckListHaveStatus(ConstUtil.Status_PieceAtk, aList))
                        && (CommonUtil.Battle_CheckListAllSpecificStatus(new int[]{ConstUtil.Status_PieceDead,ConstUtil.Status_PieceNoAtk}, bList) && !CommonUtil.Battle_CheckListHaveStatus(ConstUtil.None, bList) && !CommonUtil.Battle_CheckListHaveStatus(ConstUtil.Status_PieceAtk, bList)))
                {
                    // 结束：b 空，a 赢
                    resultComponent.status = ConstUtil.ResultStatus_AWin;
                }
                else if ((CommonUtil.Battle_CheckListAllSpecificStatus(new int[]{ConstUtil.Status_PieceDead,ConstUtil.Status_PieceNoAtk}, aList) && !CommonUtil.Battle_CheckListHaveStatus(ConstUtil.None, aList) && !CommonUtil.Battle_CheckListHaveStatus(ConstUtil.Status_PieceAtk, aList))
                        && (!CommonUtil.Battle_CheckListAllSpecificStatus(new int[]{ConstUtil.Status_PieceDead,ConstUtil.Status_PieceNoAtk}, bList) && !CommonUtil.Battle_CheckListHaveStatus(ConstUtil.None, bList) && !CommonUtil.Battle_CheckListHaveStatus(ConstUtil.Status_PieceAtk, bList)))
                {
                    // 结束：a 空， b 赢
                    resultComponent.status = ConstUtil.ResultStatus_BWin;
                }
                else
                {
                    // 未结束：走当前轮次
                    resultComponent.status = ConstUtil.ResultStatus_Unknown;
                    int b = round % 2;
                    int a = 1 - b;
                ...
                if (resultComponent.status == ConstUtil.ResultStatus_Unknown)
                {
                    round = round + 1;
        ...
        public Dictionary<int, List<Entity>> DoBattle(List<Entity> aList, List<Entity> bList)
        {
            ...
            if (atkEntity != null && defEntity == null)
            {
                ...
                for (int i = 0; i < aList.Count; i++)
                {
                    ...
                    if (status != ConstUtil.Status_PieceDead || status != ConstUtil.Status_PieceNoAtk)
                    {
