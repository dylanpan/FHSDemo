...
public class BattleAutoChessSystem : ISystem
{
    ...
    public void GetBattleResult(int round, ref Entity entity)
    {
        ...
        if (resultComponent != null)
        {
            ...
            else
            {
                ...
                List<Entity> _curRoundAList = Util.CopyList(resultComponent.result_dict[round-1][a]);
                List<Entity> _curRoundBList = Util.CopyList(resultComponent.result_dict[round-1][b]);
    ...
    public Dictionary<int, List<Entity>> DoBattle(List<Entity> aList, List<Entity> bList)
    {
        ...
        if (atkEntity != null && defEntity == null)
        {
            ...
            for (int i = 0; i < aList.Count; i++)
            {
                ...
                int status = Util.Battle_GetEntityStatus(entity);
                if (status != ConstUtil.Status_PieceDead || status != ConstUtil.Status_PieceNoAtk)
                {
                    Util.Battle_UpdateEntityStatus(entity, true);
                }
            }
        }
    ...
    public override void Update()
    {
        ...
        Entity battleAEntity = battleEntitys.Count > 0 ? battleEntitys[ConstUtil.Team_A] : Util.Battle_GetEmptyEntity();
        Entity battleBEntity = battleEntitys.Count > 1 ? battleEntitys[ConstUtil.Team_B] : Util.Battle_GetEmptyEntity();
